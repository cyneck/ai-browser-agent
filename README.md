...
| **执行层** | **指令模板库** | N/A | 各个原子动作的 `.j2` 模板文件 | Jinja2 |
| | **ActionExecutor** | 浏览器 `page` 对象、会话状态、JSON 指令 | `exec()` 执行结果 (成功/失败) | Python, `exec()`, 函数封装 |
| | **状态管理** | 各个模块的中间数据 | 持久化的浏览器会话和程序会话状态 | `launch_persistent_context`, 共享字典 |
...
## **4. 关键技术细节**
  ...
  * **会话管理**：
      ...
      * **程序会话**：使用一个共享的 Python 字典作为命名空间，在 `exec()` 调用间保持状态和变量。
  * **安全机制**：
      * **代码生成**：不直接执行 LLM 生成的代码，而是通过模板引擎渲染，确保只执行预先定义好的、安全的 Playwright 函数。
      * **沙盒化执行（未来扩展）**：在生产环境中，考虑使用如 `llm-sandbox` 等工具，将执行环境容器化，进一步隔离潜在风险。
